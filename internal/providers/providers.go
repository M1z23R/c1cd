...
// CreateGitHubCheckRun creates a new GitHub Check Run
// Returns the check run ID for later updates
func CreateGitHubCheckRun(token string, job *config.PipelineJob, commitSHA string) (int64, error) {
	parts := strings.SplitN(job.ProjectName, "/", 2)
	if len(parts) != 2 {
		return 0, fmt.Errorf("invalid GitHub repository format: %s, expected owner/repo", job.ProjectName)
	}
	owner, repo := parts[0], parts[1]
	...
	if resp.StatusCode >= 200 && resp.StatusCode < 300 {
		var checkRunResponse struct {
			ID int64 `json:"id"`
		}
		if err := json.Unmarshal(body, &checkRunResponse); err != nil {
			return 0, err
		}
		return checkRunResponse.ID, nil
	}

	return 0, fmt.Errorf("failed to create GitHub check run, status %d: %s", resp.StatusCode, string(body))
}

// UpdateGitHubCheckRun updates an existing GitHub Check Run
// States: queued, in_progress, completed
// For completed state, conclusion must be one of: success, failure, neutral, cancelled, skipped, timed_out, action_required
func UpdateGitHubCheckRun(token string, job *config.PipelineJob, checkRunID int64, state, conclusion, summary, text string) error {
	parts := strings.SplitN(job.ProjectName, "/", 2)
	if len(parts) != 2 {
		return fmt.Errorf("invalid GitHub repository format: %s, expected owner/repo", job.ProjectName)
	}
	...
	if state == "completed" {
		...
		payload["output"] = map[string]any{
			"title":   "c1cd Build Results",
			"summary": summary,
		}
		if text != "" {
			payload["output"].(map[string]any)["text"] = text
		}
	}
	...
	body, _ := io.ReadAll(resp.Body)
	return fmt.Errorf("failed to update GitHub check run, status %d: %s", resp.StatusCode, string(body))
}